...
class QutipQPU(QPUHandler):
    ...
    Args:
        ...
        hardware_model (HardwareModel, optional): the hardware model. Defaults to None.
        sim_method (str, optional): the simulation method, "deterministic" or "stochastic".
            Defaults to "deterministic".
        ntraj (int, optional): number of quantum trajectories. Defaults to 1000.
    ...
    def __init__(self,
        ...
        if sim_method not in ["deterministic", "stochastic"]:
            raise exceptions_types.QPUException(
                code=exceptions_types.ErrorType.INVALID_ARGS,
                modulename=module_name,
                message=(f"sim_method should be deterministic or stochastic, got {sim_method} "),
                file=__file__,
                line=current_line_no,
            )
    ...
    def submit_job(self, job):
        ...
        # set up the jump operators (if given) in QuTiP format.
        if self._hardware_model is not None:
            self.qutip_c_ops = [convert_obs_to_qutip(obs, self._hilbert_space) for obs in self._hardware_model.jump_operators]
        else:
            self.qutip_c_ops = []
        ...
        if job.type == ProcessingType.OBSERVABLE:
            ...
            if self._sim_method == "deterministic":
            ...
            elif self._sim_method == "stochastic":
                qt_result = qt.mcsolve(H_res_total, psi_0, time_list,
                                       c_ops=self.qutip_c_ops, e_ops=qt_e_obss,
                                       ntraj=self._ntraj,
                                       options=qt.Options(**self._qt_options) if _use_qutip_4() else self._qt_options)
